...
import { Home, FileText, ExternalLink, RefreshCw, Star, Building, MapPin, ArrowRight } from 'lucide-react';
...
const AgentDashboard = () => {
  ...
  const fetchProperties = async (email) => {
    try {
      ...
      const url = email === 'demo1.cobnb@gmail.com' 
        ? `https://creator.zoho.com/api/v2/brandontan18/housekeeping-system/report/All_Properties?Agent_Demo_Email=${encodedEmail}`
  ...
  const checkOwner = async (email) => {
    try {
      ...
      if (res?.data?.code === 3000) setCheckIsOwner(true);
    } catch (error) {
      console.log('checkOwner error', error);
    }
  };
    ...
    <div className="luxury-dashboard">
      ...
      <div className="container">
        ...
        {checkIsOwner && (
          ...
          >
            <div>
              <h4 style={{ margin: '0 0 4px 0', color: 'var(--color-primary)', fontSize: '18px', fontWeight: '600' }}>Owner Dashboard Available</h4>
              <p style={{ margin: 0, color: 'var(--color-text-muted)', fontSize: '14px' }}>You have properties managed as an Owner. Click here to switch views.</p>
            </div>
        ...
        <div className="section-container fade-in-up stagger-2">
          ...
          {loading && properties.length === 0 ? (
                ...
                <div key={index} className="property-card hover-lift" style={{ backgroundColor: 'var(--color-secondary)', borderRadius: 'var(--radius-sm)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)', display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ padding: '24px', flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                      <span style={{ fontSize: '12px', color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: '600' }}>Agent Commission</span>
                  ...
                  <div style={{ display: 'flex', borderTop: '1px solid var(--color-border)' }}>
                    ...
                    <button 
                      className="hover-lift"
                      onClick={() => navigate('/generate-statement', { state: { item, role: 2 } })}
            ...
            <div style={{ padding: '80px 20px', textAlign: 'center', backgroundColor: 'var(--color-secondary)', borderRadius: 'var(--radius-sm)', boxShadow: 'var(--shadow-sm)' }}>
              ...
              <p style={{ color: 'var(--color-text-muted)', fontSize: '16px', maxWidth: '400px', margin: '0 auto', lineHeight: '1.6' }}>You do not have any properties assigned to this agent account yet.</p>
            </div>
          )}
        </div>
      </div>
